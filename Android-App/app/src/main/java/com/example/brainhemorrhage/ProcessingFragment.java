...
public class ProcessingFragment extends Fragment {
                        ...
                        handler.postDelayed(this, 1500);
                            ...
                            if (tfliteResult != null) {
                                if (tfliteResult.validationFailed) {
                                    ...
                                    riskLevel = "N/A";
                                } else if (tfliteResult.hasHemorrhage) {
                                    result = "Abnormal";
                            ...
                            // ── VALIDATION GATE ────────────────────────────────────────────────────
                            // Do NOT upload or save invalid / errored scans to the database.
                            // Navigate directly to the result screen so the user sees the rejection.
                            if ("Invalid Image".equals(finalResult) || "Error".equals(finalResult)) {
                                handler.post(() -> navigateNext(finalResult, finalImageUri));
                                return;
                            }
                            // ── END VALIDATION GATE ────────────────────────────────────────────────
                            ...
                            if (imagePart != null) {
                                call = api.uploadScan(doctorEmailPart, patientIdPart, patientNamePart, patientAgePart, patientGenderPart, resultPart, riskLevelPart, imagePart);
                            } else {
                                call = api.uploadScan(doctorEmailPart, patientIdPart, patientNamePart, patientAgePart, patientGenderPart, resultPart, riskLevelPart, null);
                            }
    ...
    private void navigateNext(String result, String finalImageUri) {
        ...
        if (isAdded()) {
            // Save locally in SQLite Database — only for valid, processed scans
            // Invalid images and errors are intentionally excluded from history
            if ("Invalid Image".equals(result) || "Error".equals(result)) {
                // Skip DB insert; fall through to navigation below
            } else
            try {
                ...
                DatabaseHelper.getInstance(requireContext()).insertScan(
                    ...
                    currentTime,
                    result,
                    result != null && result.toLowerCase().contains("abnormal") ? (tfliteResult != null && tfliteResult.highestConfidence > 0.7f ? "CRITICAL" : "HIGH") : "LOW",
                    finalImageUri,
                    intraventricular,
                    intraparenchymal,
                    subarachnoid,
