...
public class ResultFragment extends Fragment {
    ...
    private TextView resultStatusText, confidenceText;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (patientName == null || patientName.isEmpty()) {
            android.content.SharedPreferences prefs = requireContext().getSharedPreferences("HemoScanPrefs", android.content.Context.MODE_PRIVATE);
    ...
    private void loadResultData(View view) {
        ...
        if (args != null) {
            ...
            if (scanId != null && scanId.matches("\\d+")) {
                ...
                try {
                    ...
                    if (cursor != null && cursor.moveToFirst()) {
                        ...
                        args.putFloat("confidence", maxProb > 0.0f ? maxProb : ("abnormal".equalsIgnoreCase(dbResult) ? 0.85f : 0.0f));
                        args.putInt("detectionCount", "abnormal".equalsIgnoreCase(dbResult) ? 1 : 0);
            ...
            isAbnormal = (scanId != null && (scanId.contains("critical") || scanId.equalsIgnoreCase("Abnormal"))) ||
                         (resultStr != null && resultStr.toLowerCase().contains("abnormal"));
        ...
        if (isAbnormal) {
            ...
            float confVal = args.getFloat("confidence", 0.0f);
            int count = args.getInt("detectionCount", 0);
            if (confVal > 1.0f) confVal = 1.0f; 
            
            resultStatusText.setText("ABNORMAL");
            confidenceText.setText(String.format(Locale.getDefault(), "Confidence: %.1f%%", confVal * 100));

            addFinding("AI MODEL PREDICTIONS:");
            addFinding("- Detection: Positive for Intracranial Hemorrhage");
            addFinding("- Regions Identified: " + count);
            addFinding("- Model Confidence: " + String.format(Locale.getDefault(), "%.1f%%", confVal * 100));

            addRecommendation("IMMEDIATE: Consult neurosurgeon", true);
            addRecommendation("Recommend CT angiography for vascular assessment", false);
            addRecommendation("Monitor patient for neurological changes", false);
            ...
            String errorMsg = args.getString("validationError");
            if (errorMsg != null) {
                confidenceText.setText(errorMsg);
            } else {
                confidenceText.setText("Processing Failed");
            }
            addFinding("- AI model failed to process the image");
            addFinding("- Please ensure the image is a clear brain CT scan");
            ...
            String errorMsg = args.getString("validationError", "Input rejected: not a brain CT image");
            confidenceText.setText(errorMsg);
            ...
            resultStatusText.setText("NORMAL");
            confidenceText.setText("No abnormalities detected");

            addFinding("AI MODEL PREDICTIONS:");
            addFinding("- Detection: Negative (No Hemorrhage Detected)");
            addFinding("- Regions Identified: 0");

            addRecommendation("No immediate intervention required", false);
            addRecommendation("Follow standard care protocols", false);
        }
        if (args != null && args.containsKey("intraventricular")) {
            float intraventricular = args.getFloat("intraventricular", 0.0f);
            float intraparenchymal = args.getFloat("intraparenchymal", 0.0f);
            float subarachnoid = args.getFloat("subarachnoid", 0.0f);
            float epidural = args.getFloat("epidural", 0.0f);
            float subdural = args.getFloat("subdural", 0.0f);
            
            addFinding("\nSUBTYPE PROBABILITIES:");
            addFinding("  • Intraventricular: " + String.format(Locale.getDefault(), "%.1f%%", intraventricular * 100));
            addFinding("  • Intraparenchymal: " + String.format(Locale.getDefault(), "%.1f%%", intraparenchymal * 100));
            addFinding("  • Subarachnoid: " + String.format(Locale.getDefault(), "%.1f%%", subarachnoid * 100));
            addFinding("  • Epidural: " + String.format(Locale.getDefault(), "%.1f%%", epidural * 100));
            addFinding("  • Subdural: " + String.format(Locale.getDefault(), "%.1f%%", subdural * 100));
        }
